...
def extract_water_chem(
    ...
    if len(par_list) == 1:
        sql = (
            "SELECT * FROM resa2.parameter_definitions "
    ...
    sql = (
        ...
        "  AND sample_date >= DATE '%s') "
        "AND approved = 'YES'" % (stn_df["station_id"].iloc[0], end_dt, st_dt)
    )
    ...
    sql = (
        ...
        "WHERE station_id = %s "
        "AND sample_date <= DATE '%s' "
        "AND sample_date >= DATE '%s'" % (stn_df["station_id"].iloc[0], end_dt, st_dt)
    )
    ...
    wc_df = pd.merge(
        wc_df, meth_par_df, how="left", left_on="method_id", right_on="wc_method_id"
    )
    ...
    wc_df = pd.merge(
        wc_df, samp_df, how="left", left_on="sample_id", right_on="water_sample_id"
    )
    ...
    wc_df = pd.merge(
        ...
        all_par_df,
        how="left",
        left_on="wc_parameter_id",
        right_on="parameter_id",
    )

    # Join in station codes
    wc_df = pd.merge(
        wc_df, stn_df, how="left", left_on="station_id", right_on="station_id"
    )
    ...
    dup_df = wc_df[
        wc_df.duplicated(subset=["station_code", "sample_date", "name"], keep=False)
    ].sort_values(by=["station_code", "sample_date", "name"])
    ...
    if len(dup_df) > 0:
        ...
        # Drop duplicates
        wc_df.drop_duplicates(
            subset=["station_code", "sample_date", "name"], keep="last", inplace=True
        )
    ...
    if plot:
        ...
        for pid, par in enumerate(par_list):
            ...
            axes[idx].set_title(
                "%s at %s" % (par, stn_code.decode("windows-1252")), fontsize=16
            )
            axes[idx].set_xlabel("")
            if unit:
                axes[idx].set_ylabel(
                    "%s (%s)" % (par, unit.decode("windows-1252")), fontsize=14
                )
            else:
            ...
            axes[idx].set_xlim(
                [
                    dt.datetime.strptime(st_dt, "%Y-%m-%d").date(),
    ...
    # Join
    wc_df = pd.merge(
        wc_val_df, wc_lod_df, how="left", left_index=True, right_index=True
    )
...
def extract_discharge(stn_id, st_dt, end_dt, engine, plot=False):
    ...
    sql = (
        "SELECT * FROM resa2.default_dis_stations "
    ...
    # Chemistry station
    sql = (
        "SELECT catchment_area FROM resa2.stations "
        "WHERE station_id = %s" % stn_df["station_id"].iloc[0]
    )
    area_df = pd.read_sql_query(sql, engine)
    ...
    sql = (
        ...
        "AND xdate >= DATE '%s' "
        "AND xdate <= DATE '%s'" % (dis_stn_id, st_dt, end_dt)
    )
    ...
    if plot:
        ...
        ax.set_title("Discharge at %s" % stn_code.decode("windows-1252"), fontsize=16)
        plt.tight_layout()
...
def adjust_lod_values(wc_df):
    ...
    for par_unit in par_unit_list:
        ...
        # Prop <= LOD
        p_lod = (100.0 * n_lod) / len(lod_df)

        # Adjust ALL values
        ad_val_df = (val_df / 100.0) * (100.0 - p_lod)
...
def estimate_loads(stn_id, par_list, year, engine, infer_missing=True, samp_sel=None):
    ...
    unit_dict = {
        "SPM": [1.0e9, "tonnes"],  # mg to tonnes
        "TOC": [1.0e9, "tonnes"],  # mg to tonnes
        "PO4-P": [1.0e12, "tonnes"],  # ug to tonnes
        "TOTP": [1.0e12, "tonnes"],  # ug to tonnes
        "NO3-N": [1.0e12, "tonnes"],  # ug to tonnes
        ...
        "Cd": [1.0e12, "tonnes"],  # ug to tonnes
        "Cu": [1.0e12, "tonnes"],  # ug to tonnes
        "Zn": [1.0e12, "tonnes"],  # ug to tonnes
        "Ni": [1.0e12, "tonnes"],  # ug to tonnes
        "Cr": [1.0e12, "tonnes"],  # ug to tonnes
        "Hg": [1.0e12, "kg"],  # ng to kg
        "HCHG": [1.0e15, "tonnes"],  # ng to tonnes
        "SUMPCB": [1.0e15, "tonnes"],  # ng to tonnes
    }

    # Get water chem data
    wc_df, dup_df = extract_water_chem(
        stn_id,
        par_list,
        "%s-01-01" % year,
        "%s-12-31" % year,
    ...
    if infer_missing:
        wc_df = estimate_missing_data(
            wc_df, stn_id, par_list, year, engine, samp_sel=samp_sel
        )
    ...
    if len(wc_df) > 0:
        ...
        for par_unit in par_unit_list:
            ...
            dates.append(
                np.datetime64("%s-01-01" % (year + 1))
            )  # Assumes time = midnight
            ...
            par_df[par_l] = (
                1000 * par_df["flow_m3/s"] * par_df["t_i"] * par_df[par_unit]
            )
            ...
            data_dict[var_name + "_" + unit] = (sigma_q * sum_df[par_l]) / (
                conv_fac * sigma_Qi_ti
            )
...
def estimate_missing_data(wc_df, stn_id, par_list, year, engine, samp_sel=None):
    ...
    mis_par_list = list(set(par_list) - set([i.split("_")[0] for i in wc_df.columns]))

    # Are some data missing?
    if len(mis_par_list) > 0:
        # Get all the data from 1990 to the (current_year - 1)
        wc_ts, dup_ts = extract_water_chem(
            stn_id,
            mis_par_list,
            "1990-01-01",
        ...
        for par_unit in par_unit_list:
            ...
            if len(par_ts) > 0:
                ...
                if len(par_ts) > 2:
                    ...
                    slp, incpt, lo, hi = stats.theilslopes(
                        par_ts.values[:, 0], par_ts.index.year
                    )
...
def write_csv_water_chem(
    stn_df, year, csv_path, engine, samp_sel=None, extract_flow=False
):
...
def update_cell(row_txt, par_txt, value, col_dict, row_dict, tab):
...
def write_word_water_chem_tables(
    stn_df, year, in_docx, engine, samp_sel=None, extract_flow=False
):
    ...
    if year < 2017:
        ...
        # Chem pars of interest (2017-20; includes DOC, Part. C, TDP
        # and Tot. Part. N)
        par_list = [
            "pH",
            "KOND",
        ...
        # Chem pars of interest (2021 onwards; switch TDP for TOTP/P)
        par_list = [
            "pH",
            "KOND",
            "TURB860",
    ...
    for tab_id, tab in enumerate(doc.tables):
        ...
        wc_df.rename(
            {"TOTP/P_µg/l": "Tot. Part. P_µg/l", "TOTP/P_flag": "Tot. Part. P_flag"},
            axis="columns",
            inplace=True,
        )
        ...
        if len(wc_df) > 0:
            ...
            for idx, dt in enumerate(dates):
                ...
                cell.text = pd.to_datetime(str(dt)).strftime(
                    "%d.%m.%Y"
                )  # Used 2017 onwards to save space
            ...
            blank_ids = [
                first_blank,
            ] * n_blank  # Delete first blank row n times
            ...
            for idx, df_row in df.iterrows():
                ...
                for par_unit in par_unit_list:
                    ...
                    # Update the table
                    update_cell(dt_tm, par, value, col_dict, row_dict, tab)
            ...
            for df_col in par_unit_list:
                ...
                samp_df = df[[df_col, par + "_flag"]].dropna(
                    subset=[
                        df_col,
                    ]
                )
                ...
                # Average
                lo_av = val_df.mean()
                update_cell("Lower avg.", par, lo_av, col_dict, row_dict, tab)

                # 2. Upper av. - assume all LOD values are LOD
                up_av = samp_df[df_col].mean()
                update_cell("Upper avg..", par, up_av, col_dict, row_dict, tab)

                # 3. Min
                par_min = samp_df[df_col].min()
                update_cell("Minimum", par, par_min, col_dict, row_dict, tab)

                # 4. Max
                par_max = samp_df[df_col].max()
                update_cell("Maximum", par, par_max, col_dict, row_dict, tab)

                # 5. More than 70% above LOD?
                lod_df = samp_df[par + "_flag"].values
                pct_lod = (100.0 * (lod_df == "<").sum()) / len(
                    lod_df
                )  # % at or below LOD
                ...
                if pct_lod > 70:
                    ...
                    lod_gt_70 = "no"
                update_cell(
                    "More than 70% >LOD", par, lod_gt_70, col_dict, row_dict, tab
                )

                # 6. n samples
                n_samps = len(samp_df)
                update_cell("n", par, n_samps, col_dict, row_dict, tab)

                # 7. Std. Dev.
                par_std = samp_df[df_col].std()
                update_cell("St.dev", par, par_std, col_dict, row_dict, tab)
...
def write_word_overall_table(mon_df, umon_df, in_docx):
    ...
    for tab_id, tab in enumerate(doc.tables):
        ...
        for par in par_list:
            ...
            # Update doc
            update_cell("Total Riverine Inputs", par, tot, col_dict, row_dict, tab)
            ...
            for idx, row in df.iterrows():
                ...
                # Write values
                update_cell(grp_dict[idx], par, val, col_dict, row_dict, tab)
        ...
        for par in par_list:
            ...
            if par_dict[par]:
                ...
                if par == "Flow rate":
                    ...
                    # Update values
                    update_cell("Sewage Effluents", par, sew, col_dict, row_dict, tab)

                    update_cell(
                        "Industrial Effluents", par, ind, col_dict, row_dict, tab
                    )

                    update_cell("Fish Farming", par, aqu, col_dict, row_dict, tab)

                    update_cell(
                        "Total Direct Inputs", par, tot, col_dict, row_dict, tab
                    )
    ...
    for tab_id, tab in enumerate(doc.tables):
        ...
        for par in ["Flow rate", "PO4-P", "TOTP", "NO3-N", "NH4-N", "TOTN"]:
            ...
            update_cell("Unmonitored values", par, val, col_dict, row_dict, tab)
    ...
    for tab_id, tab in enumerate(doc.tables):
        ...
        for idx, par in enumerate(par_list):
            ...
            # Write values. 'Unmonitored values' is in the Word template,
            # but the text is coloured grey so that it's invisible
            update_cell("Region values", par, tot, col_dict, row_dict, tab)
...
def write_word_overall_table_2017_20(mon_df, umon_df, in_docx):
    ...
    grp_dict = {"rid_20": "Main Rivers (20)", "rid_135": "Other Rivers (135)"}
...
def identify_point_in_polygon(
    ...
    # Spatial join
    join_gdf = gpd.tools.sjoin(pt_df2, reg_gdf, how="left", op="within")
...
def write_teotil_point_source_input_file(out_fold, ts_id, year, typ, engine):
    ...
    sql = (
        "SELECT id, regine, parameter_id, xvalue "
    ...
    # Get data for pars
    sql = "SELECT DISTINCT out_pid, name, unit " "FROM resa2.rid_punktkilder_outpar_def"
...
def write_teotil_discharge_input_file(out_fold, year, engine):
    ...
    sql = (
        "SELECT dis_station_id, ROUND(AVG(xvalue), 6) AS QAr "
    ...
    sql = (
        "SELECT dis_station_id, ROUND(AVG(xvalue), 6) AS Qsnitt "
        "FROM resa2.discharge_values "
        "WHERE dis_station_id in ( "
        "  SELECT dis_station_id "
        "  FROM resa2.discharge_stations "
    ...
    sql = (
        "SELECT dis_station_id, TO_CHAR(xdate,'MM') AS month, "
    ...
    # Combine
    q_df = pd.concat([nve_stn_df, an_avg_df, lt_avg_df, mon_avg_df], axis=1)
...
def write_teotil_observed_input_file(out_fold, loads_path, engine):
    ...
    # Get station data
    sql = (
        "SELECT station_id, station_name, nve_vassdr_nr AS regine "
        "FROM resa2.stations "
    ...
    lds_df["Hg_tonnes"] = lds_df["Hg_kg"] / 1000.0
...
def calc_productivity(row):
    ...
    if (
        (row["biomass"] == 0)
        or (row["biomass_prev"] == 0)
        or (row["biomass"] < row["biomass_prev"])
    ):
...
def estimate_fish_farm_nutrient_inputs(fish_df, year, cu):
    ...
    fish_df["biomass"] = (
        (
            (fish_df["FISKEBEHOLDNING_ANTALL"] * fish_df["FISKEBEHOLDNING_SNITTVEKT"])
            ...
            + (fish_df["TELLEFEIL"] * fish_df["FISKEBEHOLDNING_SNITTVEKT"])
        )
        / 1000.0
    ) + fish_df["UTTAK_KILO"]
    ...
    fish_sum = fish_sum.iloc[
        (fish_sum.index.get_level_values("FISKEARTID") == 71101)
    ...
    fish_sum.reset_index(inplace=True)
    fish_sum = pd.melt(
        fish_sum, id_vars="LOKNR", var_name="INP_PAR_ID", value_name="VALUE"
    )
...
def estimate_teotil_land_use_coefficients(lu_path, sheetname, year, out_fold):
    ...
    # N
    lu_df["Naker"] = (
        lu_df["N_diff"] / lu_df["Adyrket_km2"] + lu_df["N_back"] / lu_df["Adyrket_km2"]
    )
    ...
    # P
    lu_df["Paker"] = (
        lu_df["P_diff"] / lu_df["Adyrket_km2"] + lu_df["P_back"] / lu_df["Adyrket_km2"]
    )
    ...
    # Convert areas to da
    lu_df["Adyrket"] = 1000.0 * lu_df["Adyrket_km2"]
    lu_df["Aeng"] = 1000.0 * lu_df["Aeng_km2"]
...
def get_flow_volumes(stn_df, st_yr, end_yr, engine):
    ...
    for stn_id in stn_df["station_id"]:
        ...
        sql = (
            "SELECT * FROM resa2.default_dis_stations " "WHERE station_id = %s" % stn_id
        )
        dis_df = pd.read_sql_query(sql, engine)
        dis_stn_id = dis_df["dis_station_id"].iloc[0]
...
def copy_word_template(table_no, year):
    ...
    Args:
        table_no: Int. (1, 2, 3). Table to create
        year:     Int. Year of interest

    Returns:
        Str. Path to copied template.
    """
    import os
    import shutil
    ...
    if year < 2017:
        names_dict = {
            1: "rid_water_chem_tables_template.docx",
            2: "rid_loads_by_river_template.docx",
            3: "rid_loads_overall_summary_template.docx",
        }
    ...
    dest_path = os.path.join(dest_fold, f"Table_{table_no}_{year}.docx")
    Path(dest_fold).mkdir(parents=True, exist_ok=True)
    shutil.copyfile(template_path, dest_path)

    return dest_path


def copy_ospar_template(year):
    """Copy the OSPAR template.
    ...
    # Copy template
    template_path = r"../../../Results/OSPAR/Blank_Template/blank_ospar_template.xlsx"
    dest_path = f"../../../Results/OSPAR/01_OSPAR_Norway_{year}.xlsx"
    shutil.copyfile(template_path, dest_path)
